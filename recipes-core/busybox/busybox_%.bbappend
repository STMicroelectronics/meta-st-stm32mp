...
do_install:append:stm32mpcommon () {
    if [ "${@bb.utils.contains('DISTRO_FEATURES', 'systemd', '1', '0', d)}" = "0" ]; then
        if grep -q "CONFIG_IFPLUGD=y" ${B}/.config; then
            ...
            install -m 755 ${UNPACKDIR}/ifplugd.sh ${D}${sysconfdir}/init.d/ifplugd.sh
            update-rc.d -r ${D} ifplugd.sh start 99 2 3 4 5 .
            install -m 755 ${UNPACKDIR}/ifplugd.conf ${D}${sysconfdir}/ifplugd/ifplugd.conf
            install -m 755 ${UNPACKDIR}/ifplugd.action ${D}${sysconfdir}/ifplugd/ifplugd.action
        fi
    fi
}
