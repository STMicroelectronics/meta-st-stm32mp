...
# Copro Firmware load
env set coprofw_name "rproc-copro-fw.elf"
env set coprofw_addr ${kernel_addr_r}
env set boot_coprofw 'rproc init; rproc load 0 ${coprofw_addr} ${filesize}; rproc start 0'

# boot copro Firmware when available
env set scan_coprofw 'if test -e ${devtype} ${devnum}:${distro_bootpart} ${coprofw_name};then echo Found COPRO FW ${coprofw_name}; if load ${devtype} ${devnum}:${distro_bootpart} ${coprofw_addr} ${coprofw_name}; then run boot_coprofw; fi; fi;'
...
# Update the DISTRO command to search in sub-directory and load copro firmware
env set boot_prefixes "/${boot_device}${boot_instance}_"
env set boot_extlinux "run scan_coprofw;run scan_overlays; ${boot_extlinux}"
